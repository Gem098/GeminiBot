...
class EmbedUtil(commands.Cog):

    def __init__(self, bot):
        self.bot = bot

        # checks status of embedutil
    @commands.command()
    ...
    @commands.command(
        name='embed',
        description='Create stylish embeds!',
    )
    async def embed_command(self, ctx):
        """ Useful for making announcements with style. """
        ...
        await ctx.send(content='What would you like the title of the embed to be?')
        ...
        await ctx.send(content='What would you like the embed Description to be?')
        msg = await self.bot.wait_for('message', check=check)
        desc = msg.content
