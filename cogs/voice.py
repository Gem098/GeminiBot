...
class voice(commands.Cog):
            ...
            try:
                if after.channel.id == voiceID:
                    ...
                    if cooldown is None:
                        ...
                        await member.send("Creating channels too quickly you've been put on a 10 second cooldown!")
    ...
    async def voicehelp(self, ctx):
        ...
        embed.set_author(name="Voice Create", url="https://discord.gg/4BWXQcK", icon_url="https://imgur.com/c6tRF48")
    ...
    async def setup(self, ctx):
        ...
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 151028268856770560:
            ...
            await ctx.channel.send("**You will have 60 seconds to answer each question!**")
            await ctx.channel.send(
                f"**Enter the name of the category you wish to host dynamic voice channels in:(e.g Voice Channels)**")
            try:
                category = await self.bot.wait_for('message', check=check, timeout=60.0)
            except asyncio.TimeoutError:
                await ctx.channel.send('You took too long to answer! Please repeat setup!')
            else:
                new_cat = await ctx.guild.create_category_channel(category.content)
                await ctx.channel.send('**Enter the name of the main voice channel: (e.g Join To Create)**')
                try:
